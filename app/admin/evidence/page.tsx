...
export default function EvidencePage() {
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((item) => [item.name, item.email, item.location, item.subject, item.message]
    ...
    <div className="space-y-7">
      ...
      <div className="flex flex-col gap-3 sm:flex-row">
        <div className="relative flex-1">
          ...
          <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar por persona, ubicación o mensaje..." className="w-full rounded-[5px] bg-white/10 py-3 pl-11 pr-4 text-white placeholder:text-white/45 focus:outline-none focus:ring-2 focus:ring-[#4DA3D9]" />
        </div>
        <button onClick={() => void load()} disabled={loading} className="inline-flex items-center justify-center gap-2 rounded-[5px] border border-white/10 bg-white/5 px-4 py-3 text-sm text-white/70 hover:bg-white/10 hover:text-white disabled:opacity-50">
