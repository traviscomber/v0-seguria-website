...
type Incident = { id: string; assigned_to: string | null; title: string; description: string | null; severity: 'warning' | 'critical'; status: string; created_at: string; updated_at: string; properties: { name: string; address: string | null } | null; incident_actions: IncidentAction[] }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
        <section className="space-y-3">{visible.length === 0 ? <Empty text="No hay incidentes en esta vista." /> : visible.map((incident) => <button key={incident.id} onClick={() => setSelected(incident)} className={`w-full rounded-2xl border p-5 text-left transition ${selected?.id === incident.id ? 'border-[#4DA3D9]/60 bg-[#4DA3D9]/10' : 'border-white/10 bg-white/5 hover:bg-white/8'}`}><div className="flex items-start justify-between gap-3"><div><p className="text-sm text-white">{incident.title}</p><p className="mt-1 text-xs text-white/45">{incident.properties?.name || 'Sitio'} · {new Intl.DateTimeFormat('es-CL',{dateStyle:'short',timeStyle:'short'}).format(new Date(incident.created_at))}</p></div><span className={`rounded-full px-3 py-1 text-xs ${incident.severity === 'critical' ? 'bg-red-500/15 text-red-200' : 'bg-amber-500/15 text-amber-200'}`}>{incident.severity === 'critical' ? 'Crítico' : 'Atención'}</span></div><p className="mt-4 text-xs text-[#9DD2F2]">{statusLabel[incident.status]}</p></button>)}</section>
        <section className="glass-card min-h-[420px] p-6">{!selected ? <Empty text="Selecciona un incidente para gestionarlo." /> : <div className="space-y-6"><div className="flex items-start justify-between gap-4"><div><p className="text-xs uppercase tracking-[0.18em] text-white/40">{selected.properties?.name}</p><h2 className="mt-2 text-2xl font-light text-white">{selected.title}</h2><p className="mt-2 text-sm leading-6 text-white/55">{selected.description || 'Sin descripción adicional.'}</p></div><ShieldAlert className="h-7 w-7 text-[#9DD2F2]" /></div>
          <div className="grid gap-3 sm:grid-cols-3"><Info icon={Clock3} label="Estado" value={statusLabel[selected.status]} /><Info icon={UserRound} label="Responsable" value={operators.find((item) => item.id === selected.assigned_to)?.name || 'Sin asignar'} /><Info icon={AlertTriangle} label="Prioridad" value={selected.severity === 'critical' ? 'Crítica' : 'Atención'} /></div>
