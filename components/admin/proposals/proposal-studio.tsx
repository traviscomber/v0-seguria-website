...
function starterBlocks(clientName: string, context: string): ProposalBlock[] {
  return [
    { id: 'cover', type: 'cover', title: clientName ? `Propuesta de seguridad para ${clientName}` : 'Propuesta de seguridad inteligente', body: 'Una solución diseñada para proteger la operación, mejorar la visibilidad y acelerar la respuesta.', layout: 'feature', imageIds: [] },
...
export function ProposalStudio({ initialClientName = '', initialContext = '', leadId }: ProposalStudioProps) {
  ...
  useEffect(() => {
    ...
    async function restoreDraft() {
      try {
        ...
        // Local recovery remains available when the API or migration is not ready.
      }
  ...
  const runAgent = () => {
    ...
    setAgentMessage('La estructura fue actualizada con bloques autorizados por el brandbook y quedará registrada como una nueva revisión.')
    setAgentInstruction('')
  ...
  const statusText = saveStatus === 'loading'
    ...
    : saveStatus === 'saving'
      ? 'Guardando'
      : saveStatus === 'saved'
        ? `Guardado${lastSavedAt ? ` · ${new Intl.DateTimeFormat('es-CL', { hour: '2-digit', minute: '2-digit' }).format(new Date(lastSavedAt))}` : ''}`
        : saveStatus === 'error'
    ...
    <div className="space-y-6">
      <section className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm text-white/55">Edición por bloques, autoguardado, revisiones y brandbook obligatorio.</p>
        </div>
        <div className="flex items-center gap-3">
          <span className="flex items-center gap-2 text-xs text-white/45">{saveStatus === 'saving' && <Loader2 className="h-3.5 w-3.5 animate-spin" />}{statusText}</span>
          <Button onClick={() => persist('manual')} disabled={saveStatus === 'saving'} variant="outline" className="border-white/15 bg-white/[0.04] text-white hover:bg-white/10">
