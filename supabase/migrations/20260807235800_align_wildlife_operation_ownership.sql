...
-- Deterministic compatibility backfill only: no ownership is inferred for rows
-- whose operation is not linked to a property.
...
-- Tighten operation link policies to authenticated users explicitly.
...
-- Cameras: operational rows are shared read state for members of the operation.
-- Rows without an operation retain the previous personal ownership behavior.
...
-- Inference jobs follow the same scope semantics.
