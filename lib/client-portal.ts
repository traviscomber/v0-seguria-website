...
export function getPortalEvidenceGallery(site: PortalSiteSummary): PortalEvidenceGalleryItem[] {
  ...
  const incidentItems = site.incidents.flatMap((incident) => {
    ...
    return incident.evidence.slice(0, 2).map((evidence) => ({
      ...
      proof: evidence.note || evidence.fileName,
      status: incident.statusLabel,
  ...
  const documentItems = site.documents.slice(0, 4).map((document) => ({
    id: `${site.propertyId}-document-gallery-${document.id}`,
    label: 'Documento',
    ...
    tone: document.estado === 'aprobado' ? 'ok' : 'warning',
    rank: document.estado === 'aprobado' ? 42 : 58,
  } satisfies PortalEvidenceGalleryItem))
  const traceabilityItems = traceability.slice(0, 4).map((item) => ({
    id: `${item.id}-gallery`,
    label: 'Trazabilidad',
    title: item.title,
    detail: item.decisionLink,
    proof: item.evidence,
    status: item.status,
  ...
  const eventItems = site.events.slice(0, 4).map((event) => {
    ...
    return {
      ...
      title: event.title,
      detail: event.type,
      proof: event.state || 'Actividad registrada por el sitio.',
  ...
  const gallery = [...incidentItems, ...documentItems, ...traceabilityItems, ...eventItems]

  if (gallery.length === 0) {
    return [{
      id: `${site.propertyId}-empty-gallery`,
      label: 'Evidencia',
      title: 'Sin evidencia publicada',
      detail: 'Aun no hay respaldo reciente visible para este sitio.',
      proof: 'Pendiente',
      status: 'Inicial',
      action: 'Publicar la primera evidencia o documento operativo del sitio.',
      tone: 'warning',
      rank: 1,
    }]
  }
